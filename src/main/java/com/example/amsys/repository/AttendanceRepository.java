...
public interface AttendanceRepository extends JpaRepository<Attendance, AttendanceId> {
    ...
    /**
     * 指定した日付、学籍番号、コマの出席情報を取得
     * @param date 日付
     * @param studentId 学籍番号
     * @param lessontimeCode コマコード
     * @return 出席情報
     */
    @Query("SELECT a FROM Attendance a WHERE a.id.date = :date AND a.id.studentId = :studentId AND a.id.lessontimeCode = :lessontimeCode")
    Optional<Attendance> findByDateAndStudentIdAndLessontimeCode(
            @Param("date") LocalDate date,
